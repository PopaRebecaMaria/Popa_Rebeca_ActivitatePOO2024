...
public:
	...
	//cand avem prea multi op de cast-> ambiguu deci fol cuv explicit inaintea fiecaruia
	...
	//prin friend compilatorul intelege ca e o functie, nu o metoda
	//met este fct din int clasei care are pe prima poz pointerul this
	friend ostream& operator<<(ostream& output, Cofetarie c);

	//mereu prin referintqa la operatorul de citire ( >> )
	...
	//bool ala cu c1 si c2 de mai jos:
	...
	float& operator[](int index) {
		if (index >= 0 && index < this->nrAngajati) {
			...
			//in met de proecsare nu afis in consola
			//exceptie- eroare care poate fi tratata
...
int main() {
	...
	cin >> c2;
	cout << endl << c2 << endl;
	//operator de cast la int si la float
	int nrAngajati = (int)c2;
	float salariiTotale = (float)c2;
	//imi ret boolean: operatori binari poate fi facut sub forma de met
	if (c < c2) {
		//c1.operator<(c2);
	...
	//ca sa nu scriu c2.getSalariu(1) fol op index
	//fol op index at cand in int clasei resp avem un vector
	//index poate fi supraincarcat DOAR prin metoda(in int clasei)

	try {
		cout << endl << c2[1] << endl; //daca asta intra in catch,nu o sa mai treaca la urm lucru din try
		cout << c2[-6];
	...
	//pt a-l modofica adaugam & la operator cu index
	cout << c2[1] << endl;
	c2[1] = 2000;
	cout << c2[1] << endl;
}